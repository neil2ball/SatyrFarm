...
//  Animal feeder
// Change log:
//  Add "watering hole" option to make a lake a perpetual water supply.
//

float VERSION = 5.1;   // 04 November 2023
...
string TXT_WATER_HOLE_ON = "+WaterHole";
string TXT_WATER_HOLE_OFF = "-WaterHole";
string TXT_WATERING_HOLE = "Watering hole";
...
default
{
	listen(integer c, string nm, key id, string m)
	{
		...
		else if (m == TXT_WATER_HOLE_ON || m == TXT_WATER_HOLE_OFF)
		{
			waterHole =  (m == TXT_WATER_HOLE_ON);
			if (waterHole == 1) llSay(0, TXT_WATERING_HOLE + "=" +TXT_ON); else llSay(0, TXT_WATERING_HOLE + "=" +TXT_OFF);
			llSetTimerEvent(1);
			refresh();
		}
	...
	touch_start(integer n)
	{
		...
		if (llSameGroup(toucher) || osIsNpc(toucher))
		{
			...
			if (waterHole) opts += TXT_WATER_HOLE_OFF; else opts += TXT_WATER_HOLE_ON;
